...
class Maze():
    ...
    def generateMazeImage(self,display):
        ...
        squaresize = screenrect.w / (self.size[0] + (self.linewidth*self.size[0]) + self.linewidth)
        ...
        if squaresize*(1+self.linewidth)*self.size[1] + squaresize*self.linewidth > screenrect.h:
            squaresize = screenrect.h / (self.size[1] + (self.linewidth*self.size[0]) + self.linewidth)
            self.longside = 1
        squaresize = int(squaresize)
        
        widthspace = screenrect.w - (squaresize*(1+self.linewidth)*self.size[0] + squaresize*self.linewidth)
        heightspace = screenrect.h - (squaresize*(1+self.linewidth)*self.size[1] + squaresize*self.linewidth)

        
        self.walls = pygame.sprite.Group()
        self.walls.add(Wall(0,0,widthspace/2,screenrect.h,red))
...
testmaze = Maze(screen,(3,5),(),(),(),1/6,blue,green)
#testmaze.printself()
